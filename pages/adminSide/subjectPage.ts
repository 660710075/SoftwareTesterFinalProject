...
export class SubjectPage {
  ...
  constructor(page: Page, request: APIRequestContext) {
    ...
    this.tabMenu = page.locator(".list-group a");
    ...
    this.searchSubjectField = page.getByRole("textbox", {
      name: "ค้นหารหัสวิชา หรือ ชื่อวิชา...",
    });
    this.deleteBTN = page.getByRole("button", { name: "ลบ" });
  }

  async selectMenuTab(tabname: string) {
    await expect(this.tabMenu.filter({ hasText: tabname })).toBeVisible();
  ...
  async clickSubjectSearchResult(name: string) {
    if (
      (await this.page.getByRole("button", { name: `${name}` }).count()) > 1
    ) {
      await this.page
